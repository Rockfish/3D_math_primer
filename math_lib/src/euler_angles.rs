...
impl EulerAngles {
    ...
    pub fn from_object_to_inertial_quaternion(q: Quaternion) -> EulerAngles {
        ...
        if sp.abs() > 0.9999 {
            EulerAngles {
                ...
                heading: atan(-q.x * q.z + q.w * q.y, 0.5 - q.y * q.y - q.z * q.z),
                bank: 0.0,
            }
        } else {
            ...
            EulerAngles {
                pitch: sp.asin(),
                heading: atan(q.x * q.z + q.w * q.y, 0.5 - q.x * q.x - q.y * q.y),
                bank: atan(q.x * q.y + q.w * q.z, 0.5 - q.x * q.x - q.z * q.z).atan(),
            }
        }
    }
    ...
    pub fn from_inertial_to_object_quaternion(q: Quaternion) -> EulerAngles {
        ...
        if sp.abs() > 0.9999 {
            ...
            EulerAngles {
                pitch: sp.asin(),
                heading: atan(q.x * q.z - q.w * q.y, 0.5 - q.x * q.x - q.y * q.y),
                bank: atan(q.x * q.y - q.w * q.z, 0.5 - q.x * q.x - q.z * q.z),
            }
        }
    }
    ...
    pub fn from_world_to_object_matrix(m: Matrix4x3) -> EulerAngles {
        ...
        if sp.abs() > 9.99999 {
            ...
            EulerAngles {
                heading: atan(m.m13, m.m33).atan(),
                pitch: sp.asin(),
                bank: atan(m.m21, m.m22),
            }
        }
    }
